...
const construct = (el: CurvedTextWidget) => {
	...
	const initialiseChars = () => {
		...
		const char = el.getElementsByClassName('char') as TextElement[]; // single char textElements
		const y = radius < 0 ? -radius : -radius + char[0].getBBox().height / 2; //define y of text based on radius, and prevent mirroring
	...
	const radius = radiusEl.r ?? 100; //if negative, text is bottom curve. Default to 100.

	let textAnchor: string;
	try {
		// textEl.textAnchor throws an error if textAnchor not defined
		textAnchor = textEl.textAnchor;
	} catch (e) {
		textAnchor = 'middle'; // default
	}

	const letterSpacing: number = textEl.letterSpacing ?? 0;
	...
	let anchorAngle = 0; // angle by which whole string should be rotated to comply with anchor, excluding startAngle adjustment of anchor // former stringAngle
	...
	Object.defineProperty(el, 'startAngle', {
		// name compatible with SVG/CSS attribute
		set: function (newValue) {
			setStartAngle(newValue);
		},
	});

	Object.defineProperty(el, 'anchorAngle', {
		// name descriptive of use; function is identical to startAngle
		set: function (newValue) {
			setStartAngle(newValue);
		},
	});
	...
	el.redraw = () => {
		...
		//VARIABLES
		//ASSIGN CHARS
		const chars = textEl.text.split(''); // array of char set of text to curve
		const char = el.getElementsByClassName('char') as TextElement[]; // single char textElements
		...
		for (let i = numChars; i < char.length; i++) char[i].style.display = 'none';
		...
		const outerGAngle =
			el.parent?.['groupTransform'] === undefined
				? 0
				: (el.parent as GroupElement).groupTransform.rotate.angle;
		alignRotate.groupTransform.rotate.angle = -outerGAngle; // so getBBox() will return unrotated widths

		if (!sweepAngle) {
			// sweepAngle wasn't specified, so do mode=0 (auto)
			...
			for (let i = 0; i < numChars; i++) {
				...
				const charWidth = char[i].getBBox().width;
				cumWidth += charWidth;

				//ROTATION PER CHAR
				charG.groupTransform.rotate.angle =
			...
			switch (textAnchor) {
				case 'middle':
					anchorAngle =
						(-(cumWidth + (numChars - 1) * letterSpacing) * degreePx) / 2; //ok
					break;
				case 'end':
			...
			// sweepAngle is non-zero, so do mode=1 (fix)
			...
			(
				char[numChars - 1].parent as GroupElement
			).groupTransform.rotate.angle = 0;
			...
			switch (textAnchor) {
				...
				case 'middle':
					anchorAngle -=
						((numChars - 1) * sweepAngle +
							((lastChar - firstChar) / 2) * degreePx) /
						2;
				...
				case 'end':
					anchorAngle =
						(numChars - 1) * -sweepAngle - (lastChar / 2) * degreePx;
...
export const curvedText = (): { name: string; construct: typeof construct } => {
